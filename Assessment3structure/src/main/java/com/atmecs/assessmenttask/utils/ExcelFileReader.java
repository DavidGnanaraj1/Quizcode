...
public class ExcelFileReader {
	...
	/**
	 * This method is used to get the values of the complete row by using the
	 * sheetname,rowname as an input
	 */
	public String[] gettingValuesOfARow(String sheetName, int rowNumber) {
		int columnNumber = 0;

		String excelColumnData[] = new String[countingTheNumberOfColumns(sheetName, rowNumber)];

		for (int cellIndex = 0; cellIndex < countingTheNumberOfColumns(sheetName, rowNumber); cellIndex++) {

			excelColumnData[cellIndex ] = workbook.getSheet(sheetName).getRow(rowNumber).getCell(cellIndex+1)
					.getStringCellValue();
